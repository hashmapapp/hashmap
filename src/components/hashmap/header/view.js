...
const header = ({ data, currentUser, isAuthor, notifications }) => {
  const [info, setInfo] = useState('');
  const [hasSolicitation, setHasSolicitation] = useState(false);
  const [currentUserCreateNotification, setCurrentUserCreateNotification] = useState(false);
  ...
  useMemo(() => {
    ...
    setHasSolicitation(notifications.length > 0)
    if (currentUser) {
      notifications.map(item => {
        if (item.creatorId === currentUser.uid) {
          setCurrentUserCreateNotification(true)
        }
      })
    }
  ...
  return (
    <header>
      ...
      {modalCollaborateIsOpen &&
        <CollaborateModal
          closeModal={() => setModalCollaborateIsOpen(false)}
          hashmapKey={data.hashmapKey}
        />
      }
      {modalCopyIsOpen &&
        <CopyModal
          closeModal={() => setModalCopyIsOpen(false)}
          hashmapKey={data.hashmapKey}
        />
      }
    </header>
